...
async function runAI(publish=false){
  ...
  const pack=await window.BS_AI.run(DATA.weather, DATA.points, {yesterdayIncidents:yesterday, expectedVisitors:1200});
...
function renderAdmin(){menu(); pageNav(); if(location.hash==='#login'||sessionStorage.getItem('bsAdmin')!=='1'){showLogin('admin'); return} $('#adminShell')?.classList.remove('hidden'); $('#adminName')&&( $('#adminName').textContent=DATA.supervisor.name); map($('#adminMap'),{}); renderTables(); bindForms(); const ai=$('#aiResult'); if(ai) ai.innerHTML=aiHtml();}
...
function renderTables(){ const roster=$('#roster'); if(roster){roster.innerHTML=DATA.points.map(p=>`<tr><td>${p.no}</td><td>${p.title}</td><td><span class="tag ${riskClass(p.risk)}">${riskText(p.risk)}</span></td><td>${guardName(p.guard)}</td><td>${p.instruction}</td></tr>`).join('')}
 ...
 $('#requestsTable')&&($('#requestsTable').innerHTML=DATA.requests.map(x=>`<tr><td>${x.time}</td><td>${x.by}</td><td>${x.type}</td><td>${x.note}</td><td><span class="tag caution">جديد</span></td></tr>`).join(''));
}
...
function csv(){const rows=[['type','time','by','point','request_or_incident','note'],...DATA.incidents.map(x=>['incident',x.time,x.by,x.point,x.type,x.note]),...DATA.requests.map(x=>['request',x.time,x.by,'',x.type,x.note])];return rows.map(r=>r.map(v=>'"'+String(v||'').replaceAll('"','""')+'"').join(',')).join('\n')}
